...
const Pagination = ({page, count}: {page: number, count: number}) => {
    ...
    <div className="p-4 flex items-center justify-between text-gray-500">
        ...
        <div className="flex items-center gap-2 text-xl underline">
            {Array.from({length: Math.ceil(count / ITEM_PER_PAGE)}, (_, index)=>{
                const pageIndex = index + 1;
                return <button key={pageIndex} className={`px-2 rounded-sm ${page === pageIndex ? "bg-uSky" : ""}`} onClick={()=>{changePage(pageIndex)}}>{pageIndex}</button>
            })}
