...
const InputField = ({label, type = "text", register, name, defaultValue, error, inputProps, hidden = false}: InputFieldProps) => {
    return ( <>
        <div className={hidden ? "hidden" : "flex flex-col gap-2 w-full md:w-1/4"}>
        <label htmlFor="username" className="text-xs text-gray-500">{label}</label>
